...
fn test_page_mapper(page_mapper: &mut PageMapper, frame_allocator: &mut FrameAllocator, multiboot_addr: usize) {
    ...
    page_mapper.map(test_page, test_frame, frame_allocator).unwrap();
    log!("Success!");
    ...
    *test_value = 100;
    ...
    log!("unmapping page");
    page_mapper.unmap(test_page, test_frame, frame_allocator).unwrap();
    ...
    assert!(page_mapper.is_mapped(mboot_page), "Multiboot struct should be identity mapped.");
}
...
pub fn init(multiboot_addr: usize, _kernel_end: usize) {
    ...
    test_page_mapper(&mut page_mapper, &mut frame_allocator, multiboot_addr);
    ...
    for entry in mmap_iter {
        log!("size: {}, base_addr: 0x{:x}, length: 0x{:x}, entry_type: {}",
             entry.size(), entry.base_addr(), entry.length(), entry.entry_type());
    }
}
